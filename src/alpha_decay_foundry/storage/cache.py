...
Cache layout (see PRD §5.1)::
    ...
    ├── <source>/<dataset>/year=2024/month=01/data.parquet  # partitioned
    └── metadata/
        └── downloads.duckdb
"""
...
class CacheLayer:
    ...
    def load(
        ...
        Args:
            ...
            filters: Optional column-equality filters applied after load
                (``{column: value}``).  Passed through as pandas boolean
                masks, not as Parquet predicate pushdown.
    ...
    def query(self, sql: str) -> pd.DataFrame:
        ...
        also access Parquet files via DuckDB's ``read_parquet()`` or
        ``parquet_scan()`` table functions.
