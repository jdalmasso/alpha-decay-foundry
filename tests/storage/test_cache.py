"""Tests for src/alpha_decay_foundry/storage/cache.py."""
...
def test_store_creates_parquet_file(
    ...
    expected = tmp_path_for(cache) / "french" / "ff5_factors" / "snapshot_2020-01-01.parquet"
...
def test_store_is_atomic(cache: CacheLayer, sample_df: pd.DataFrame) -> None:
    """No partial file left behind on successful write."""
...
def test_load_with_filter_returns_subset(
    cache: CacheLayer,
) -> None:
...
def test_close_is_idempotent_via_context_manager(tmp_path: Path) -> None:
    """CacheLayer.close() releases the DuckDB connection."""
...
def tmp_path_for(cache: CacheLayer) -> Path:
